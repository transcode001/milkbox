...
- Mobile ルーティング（Bottom Tabs）
  ...
  - AddTask: サブタスク登録、タスク（カテゴリ）選択・追加・削除、タスク指定あり（曜日繰り返しモード）とタスク指定なし（日付モード）の切替、曜日選択または開始日・終了日設定、バリデーション（日付整合性・曜日未選択チェック）、登録後の遷移選択。
  - Calendar: 月次カレンダー表示、曜日繰り返し・期間タスクの Gantt バー表示、単日タスクのドット表示、選択日の予定一覧表示（全種別を合算）、当日ジャンプ。
