...
- `SavedItem`
  - 型: `id: number, categoryId?: number, text: string, date: string, startDate?: string, endDate?: string, weekdays?: string, categoryName?: string`
...
- `useCallback`
  - `loadItems` を `useCallback` でラップ（依存: `dbManager`）
- `useFocusEffect`
  - フォーカス時に `void loadItems()` を実行（依存: `loadItems`）
- Context
...
- エラーハンドリング
  - データ読込失敗時に `errorMessage = "タスクの読み込みに失敗しました"`
  - UI にエラーテキストを表示
- 表示フォールバック
  - 日付不正値または未設定時は `-` を表示
...
- 一覧はカテゴリ名でグルーピングされ、カテゴリ未設定データは常に「タスク指定なし」セクションに寄せられる。
